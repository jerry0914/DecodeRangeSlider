...
//    set_motor_direction(LOW);
    ...
    if(get_forceshift()){
      if(forceshift_counter<forceshift_steps){
        if(get_max_limiter_state() && forceshift_counter>FORCESHIFT_LIMITER_BUFFER_STEPS){
          set_forceshift(false);
          set_error(true);
          running_description = "Maximum limiter is hitted!!!";
          set_finished();
        }
        else if(get_min_limiter_state() && forceshift_counter>FORCESHIFT_LIMITER_BUFFER_STEPS){
          set_forceshift(false);
          set_error(true);
    ...
    //******************************* Stage=RUNNING *********************************
      ...
      if(get_not_initialized())
      {
        ...
        if((target_position - current_position)>0.1){
          ...
          else if(get_min_limiter_state() && current_position > LIMITER_BUFFER){
            set_error(true);
            running_description += "Minimum limiter is hitted!!!; ";
            set_finished();
          }
        ...
        // ------------------------- Moving backward --------------------------
          ...
          if(get_min_limiter_state() & current_position > LIMITER_BUFFER){
            set_error(true);
            running_description += "Minimum limiter is hitted!!!; ";
            set_finished();
          }
          else if(get_max_limiter_state() & (RAIL_MAX_RANGE-current_position)>LIMITER_BUFFER){
            set_error(true);
            running_description += "Maximum limiter is hitted!!!; ";
            set_finished();        
          }
